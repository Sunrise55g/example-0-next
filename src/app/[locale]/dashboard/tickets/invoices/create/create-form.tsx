...
export default function CreateForm(
  ...
  const initialState = { message: '', errors: {}, redirectToId: undefined };
  const [state, formAction]: any = useActionState(action, initialState);


  useEffect(() => {
    if (state.redirectToId) {
      router.push(`/dashboard/tickets/invoices/${state.redirectToId}/edit`);
    }
  }, [state.redirectToId, router]);
  ...
  async function action(prevState: IState, formData: FormData) {
    ...
    return {
      ...
      redirectToId: serviceResponse.id,
    };
  }
